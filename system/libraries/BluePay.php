...
class CI_BluePay {
    ...
    public function __construct() {
        $this->accountID = "100746732112";
        $this->secretKey = "K71NFVOULCMSJVYC6RNNIOFF6MG0GPNZ";
        $this->mode = "TEST";
    }
...
//    }
    ...
    public function setCardTypes() {
        $creditCards = 'vi-mc';
        if ($this->discoverImage == 'discvr.gif')
            $creditCards .= '-di';
        if ($this->amexImage == 'amex.gif')
            $creditCards .= '-am';
        return $creditCards;
    }
    ...
    public function addDefProtectedStatus($string) {
        if ($this->protectAmount == 'Yes')
            $string .= ' AMOUNT';
        if ($this->rebillProtect == 'Yes')
            $string .= ' REBILLING REB_CYCLES REB_AMOUNT REB_EXPR REB_FIRST_DATE';
        if ($this->protectCustomID1 == 'Yes')
            $string .= ' CUSTOM_ID';
        if ($this->protectCustomID2 == 'Yes')
            $string .= ' CUSTOM_ID2';
        return $string;
    }
    ...
    public function addStringProtectedStatus($string) {
        if ($this->protectAmount == 'Yes')
            $string .= $this->amount;
        if ($this->rebillProtect == 'Yes')
            $string .= $this->doRebill . $this->rebillCycles . $this->rebillAmount . $this->rebillExpr . $this->rebillFirstDate;
        if ($this->protectCustomID1 == 'Yes')
            $string .= $this->customID1;
        if ($this->protectCustomID2 == 'Yes')
            $string .= $this->customID2;
        return $string;
    }
